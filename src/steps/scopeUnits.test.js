...
describe('scopeLines', () => {
  it('gives every rack a rig and a commission row', () => {
    const lines = scopeLines({ racks: 2 }, U);
    expect(lines.map(l => l.desc)).toEqual([
      'Rig & set rack 1', 'Commission rack 1',
      'Rig & set rack 2', 'Commission rack 2',
    ]);
  });

  it('reads in the order the work happens', () => {
    // A list an estimator can check against a schedule. You rig it before you
    // commission it.
...
describe('scopeManHours', () => {
  ...
  it('is most of a crew-week the app used to leave out entirely', () => {
    // The point of the whole module. Two racks is 76 man-hours of
    // commissioning and rigging that was previously charged at nothing.
...
describe('scopeTasks', () => {
  ...
  it('splits man-hours across the crew rather than billing one man', () => {
    ...
    const [rig] = scopeTasks({ counts: { racks: 1 }, units: U, crewSize: 2, uid });
    expect(rig.men * rig.hrs).toBeCloseTo(2.0, 1);
  ...
  it('adds only the new rack when the count goes up', () => {
    ...
    const second = scopeTasks({ counts: { racks: 2 }, units: U, existing: first, uid });
    expect(second.map(t => t.desc)).toEqual(['Rig & set rack 2', 'Commission rack 2']);
  });
  ...
  it('does not collide with a row the estimator typed himself', () => {
    ...
    expect(out.map(t => t.desc)).toEqual(['Rig & set rack 1']);
  });
...
describe('the scope units', () => {
  ...
  it('carries provenance for all three, because two of them are contested', () => {
    for (const k of SCOPE_UNIT_KEYS) {
      expect(provenanceOf(k).note, k).toBeTruthy();
    }
  });

  it('marks the rack set and the walk-in panel DISPUTED, naming both figures', () => {
    // 2.0 hr against 16-24 hr, and 0.45 hr against 1.5-2 hr. An app that
    // silently picks one is taking a decision it is not qualified to take.
    expect(provenanceOf('perRackSet').state).toBe('disputed');
    expect(provenanceOf('perRackSet').note).toMatch(/2\.0 hr/);
    expect(provenanceOf('perRackSet').note).toMatch(/16-24 hr/);
  ...
  it('says WHY the named estimator wins the default', () => {
    ...
    expect(provenanceOf('perRackSet').note).toMatch(/named estimator/i);
  });
  ...
  it('keeps the rack TIE and the rack SET apart', () => {
    ...
    expect(provenanceOf('perRackTie').note).toMatch(/not rigging the rack itself/i);
