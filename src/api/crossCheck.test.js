...
describe('crossCheckDiff', () => {
  ...
  it('stays quiet when the two models agree', () => {
    ...
    expect(crossCheckDiff(both, { equipment: eq('RTU-1', 'RTU-2'), airDevices: eq('SD-01') })).toEqual([]);
  });
  ...
  it('handles a missing second read', () => {
    expect(crossCheckDiff({ equipment: eq('RTU-1') }, null)).toEqual([]);
    expect(crossCheckDiff(null, { equipment: eq('RTU-1') })).toEqual([]);
  });
});
